...
[role="lead"]
Diagnostiquez les défaillances d’intégration LLM dans le déploiement local de NetApp Console afin de rétablir l’accès à l’assistant de Console et des réponses fiables pour le guidage et les opérations de stockage.
...
| Le bouton *Console assistant* est manquant | État d'intégration non valide | Sauvegarde réussie des outils d'IA, état d'intégration et rôle d'accès attendu | Actualisez la session et vérifiez avec un compte administrateur valide 
