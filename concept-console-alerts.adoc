...
Le déploiement local sur console centralise les alertes des clusters ONTAP et des opérations de sauvegarde et de restauration dans une vue unique. La page Alertes comprend un tableau de bord, une liste d’alertes et une vue affichant les alertes de tous les systèmes de l’organisation. Chaque alerte précise le système concerné, son niveau de gravité et sa zone d’impact.
...
* *Gravité* indique le degré d'urgence, du plus élevé au plus faible : Critique, Avertissement et Information.
* *Zone d'impact* identifie le domaine concerné : disponibilité, capacité, configuration, performance, protection et sécurité.
...
* *Les alertes ONTAP* proviennent du système de gestion des événements ONTAP (EMS) sur les clusters sur site découverts par le déploiement local de Console, ainsi que des métriques de surveillance collectées à partir d’ONTAP. link:https://docs.netapp.com/us-en/ontap-ems/acp-auto-events.html["En savoir plus sur le ONTAP EMS et les alertes disponibles."^]
...
* Cela s'applique à tous les systèmes de l'organisation.
...
* link:task-alert-rules-manage.html["Créer et gérer des règles de notification"]
