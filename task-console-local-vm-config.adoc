...
summary: 'Vous pouvez modifier votre hôte VCenter ou ESXi existant après avoir déployé le déploiement local de Console. Par exemple, vous pouvez augmenter le CPU ou la RAM de l"instance de machine virtuelle qui héberge le déploiement local de Console.' 
---
...
. Ouvrez le client vSphere et connectez-vous à votre vCenter.
. Sélectionnez l'instance de machine virtuelle qui héberge le déploiement local de la Console.
. Cliquez avec le bouton droit sur l'instance de machine virtuelle et sélectionnez *Modifier les paramètres*.
. Augmentez l'espace disque utilisé pour /opt ou la partition /var.
+
.. Sélectionnez *Disque dur 2* pour augmenter l'espace disque utilisé pour /opt.
.. Sélectionnez *Disque dur 3* pour augmenter l'espace disque utilisé pour /var.
...
. Ouvrez le client vSphere et connectez-vous à votre vCenter.
. Sélectionnez l'instance de machine virtuelle qui héberge le déploiement local de NetApp Console.
. Sélectionnez *Lancer la console Web*.
. Connectez-vous à l'instance de machine virtuelle à l'aide du nom d'utilisateur et du mot de passe que vous avez spécifiés lors de la création de l'instance de machine virtuelle. Le nom d'utilisateur est  `maint` et le mot de passe est celui que vous avez spécifié lors de la création de l'instance de machine virtuelle.
. Accédez au  `1 to view the `System Configuration` menu.
. Saisissez  `2` et suivez les instructions à l'écran. La console recherche de nouveaux paramètres et augmente la taille des partitions.
